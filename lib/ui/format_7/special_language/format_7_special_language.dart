...
class Format7SpecialLanguage extends StatefulHookConsumerWidget {
  const Format7SpecialLanguage({super.key});

  @override
  ConsumerState<Format7SpecialLanguage> createState() =>
      _Format7SpecialLanguageState();
}

class _Format7SpecialLanguageState
    extends ConsumerState<Format7SpecialLanguage> {
  @override
  Widget build(BuildContext context) {
    final audioPlayer = ref.watch(audioPlayerOnceProvider);
    final session = ref.watch(sessionManagerProvider);
    ValueNotifier<int?> listId = useState(null);
    final teacher = ref.watch(sessionManagerProvider).getTeacherProfile();
    final specialItems = ref.watch(specialListFormatLanguageProvider(
        FilterClass(
            format: Formats.format7, classId: 0, schoolId: listId.value ?? 0)));
    var audioModeList = [
      AudioMode.english,
      AudioMode.chinese,
    ...
    ref.listen(specialListLanguageProvider(Formats.format7), (previous, next) {
      if (next.asData?.value != null) {
        final val = next.asData!.value;
        var session = teacher?.formatSevenSession ?? 1;
        var total = 1;
        for (var element in val) {
          total += element.updateFrequency;
          if (session < total) {
    ...
    ref.listen(
        specialListFormatLanguageProvider(FilterClass(
            format: Formats.format7,
            classId: 0,
            schoolId: listId.value ?? 0)), (previous, next) {
      if (next.asData?.value != null) {
                                    ...
                                    : MasonryGridView.count(
                                        ...
                                        itemBuilder:
                                          ...
                                          return SpecialLanguageItem(
                                            languageModel: item,
                                            closeTapped: () {},
                                            isLoop: currentCardLoop.value?.id ==
                                                item.id,
                                            cardLoopTapped: () {
...
class ImageView extends StatelessWidget {
  ...
  const ImageView({super.key, required this.imageUrl, required this.callback});
